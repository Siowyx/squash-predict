...
server <- function(input, output, session) {
  ...
  localreactive <- reactiveValues(model=models1[[1]])
    ...
    if (input$player1 != "NA" & input$player2 != "NA") {
      if (input$player1 == input$player2) {
        ...
        scores <- factor(c("3-0", "3-1", "3-2", "2-3",  "1-3", "0-3"), 
                            levels = c("3-0", "3-1", "3-2", "2-3",  "1-3", "0-3"))
        probs <- c(probs[7], probs[11], probs[14], probs[15], probs[13], probs[10])
        df <- data.frame(scores, probs)
        
        output$results <- renderPlot({
          ggplot(df, aes(x=scores, y=probs)) + 
            geom_bar(stat = "identity", fill = "#a6cbee") +
            geom_text(aes(label = round(probs*100, 2)), position=position_dodge(width=0.9), vjust=-0.25) +
            xlab("Scores") +
            ylab("Likelihood (%)")
        })
      }
